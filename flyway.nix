...
stdenv.mkDerivation (finalAttrs: {
  ...
  src = fetchurl {
    ...
    sha256 = "sha256-LGpTuo0ZimNeMJsdU7QhrIbo0rqKkVg8vzJWaOYSJ3U=";
  };
  nativeBuildInputs = [ makeWrapper ];
  dontBuild = true;
  dontStrip = true;
  installPhase = ''
    mkdir -p $out/bin $out/share/flyway/{jars,sql}
    cp -r drivers conf $out/share/flyway
    install -Dt $out/share/flyway/lib lib/flyway/*.jar lib/*.jar lib/oracle_wallet/*.jar
